...
| age  | `age --version`  | Key management          |

> **Node.js, npm, and git** are required for all development — building, unit testing, and linting. **sops and age** are only required for integration tests and for actually running `clef` commands against real encrypted files. Unit tests mock all subprocess calls and do not need sops or age installed.

### Platform-specific install commands

```bash
# macOS
brew install node sops age

# Ubuntu / Debian
sudo apt install nodejs npm
# sops: download from https://github.com/getsops/sops/releases
# age:  sudo apt install age  (Ubuntu 22.04+) or download binary
#       from https://github.com/FiloSottile/age/releases
...
- **Important:** if you also have Clef installed via Homebrew (`brew install clef-sh/tap/clef-secrets`), the two installations will conflict — `which clef` shows which one wins based on PATH order. Use direct invocation to avoid ambiguity.
...
Everything is mocked. No sops, no age, no git, no network. These tests run identically on any machine with Node installed.
...
### Tier 3 — Integration tests (requires sops + age)

```bash
npm run test:integration
```

Integration tests create a temporary git repository, generate a temporary age key pair, create a real `clef.yaml` manifest, and run actual SOPS encrypt/decrypt operations against real files. They verify end-to-end behaviour that unit tests cannot cover.

Prerequisite: sops and age must be installed and on PATH. No environment variables need to be set — the integration test setup script generates its own temporary key pair.

If sops or age are not installed, the suite exits with:

```
Integration tests require sops and age.
Install with: brew install sops age
```
...
# Store outside the repo to avoid accidental commits
age-keygen -o ~/clef-dev-key.txt
# Prints the public key: age1...  — save this value

# 2. Tell SOPS where the private key is
# Option A — key file path (recommended, persists across sessions):
export SOPS_AGE_KEY_FILE=~/clef-dev-key.txt
# Option B — key value inline (good for one-off sessions):
export SOPS_AGE_KEY=$(grep -v '^#' ~/clef-dev-key.txt)

# 3. Create a playground directory
mkdir -p /tmp/clef-playground && cd /tmp/clef-playground
git init

# 4. Create a minimal clef.yaml
cat > clef.yaml << 'EOF'
version: 1
environments:
  - name: dev
...
sops:
  default_backend: age
  age:
    recipient: age1...  # paste your public key from step 1
...
- `~/clef-dev-key.txt` contains your private key — store it outside any repository directory. Never commit it.
- `SOPS_AGE_KEY_FILE` is the recommended approach. `SOPS_AGE_KEY` works but exposes the raw private key in your shell environment and process table.
- The `/tmp/clef-playground` directory is safe for experimentation — cleaned up on system restart.
- If you see `could not decrypt` errors, the age recipient in `clef.yaml` does not match the key you exported. Run `clef doctor` to diagnose.
...
| Integration tests fail — sops not found          | sops not installed                 | `brew install sops age`                                                                      |
| Integration tests fail — could not decrypt       | age binary missing                 | `age --version` to verify; reinstall if missing                                              |
