/**
 * TS mirror of the @theme block in `./theme.css`.
 *
 * Why it exists: the Clef local UI is mid-migration from inline `style={{}}`
 * to Tailwind classes. Inline-style call-sites need hex literals at runtime;
 * Tailwind classes resolve from the CSS @theme block at build time. Both
 * shapes must agree, so this file mirrors the CSS values 1:1.
 *
 * Once `packages/ui/` finishes migrating, screens won't import from here —
 * they'll use Tailwind class names directly. The TS export will only be
 * needed for: dynamic colors in computed styles (e.g. SVG fills picked at
 * runtime) and the small handful of screens that materially need a JS-side
 * color reference (status pickers, severity meta maps).
 *
 * Naming convention is camel-flat (`ink950`, `gold500`) so inline-style
 * sites stay terse: `theme.gold500` vs `theme.gold[500]`. Matches the
 * shape that already exists at `packages/ui/src/client/theme.ts`.
 */
export const theme = {
  ...
  // screens (pre-migration inline styles) don't break.
  bg: "#06070b", // → ink950
...
/**
 * Radius scale (pixels). Matches the @theme `--radius-*` tokens.
 */
export const radius = {
  sm: 4,
  md: 6,
...
/**
 * Shadow recipes (strings). Matches the @theme `--shadow-*` tokens.
...
/**
 * Spacing scale (pixels). Adopt this in new UI rather than ad-hoc paddings.
 */
export const space = {
  1: 4,
  2: 8,
...
/** Per-environment color/badge meta. Local UI only — cloud has its own. */
...
/** Severity meta for lint/scan badges. */
...
/** Category meta for lint groupings. */
