...
> The formula is named `clef-secrets` to avoid a naming collision with an existing Homebrew formula. The installed binary is `clef`.
...
- **age** — `brew install age` (macOS) or see [installation guide](./installation.md)
...
::: tip
If you do not yet have an age key, `clef init` will warn you and skip file scaffolding. Generate a key with `age-keygen -o .sops/keys.txt`, then run `clef lint --fix` to scaffold the missing files.
:::
