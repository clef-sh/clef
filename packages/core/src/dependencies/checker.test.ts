...
describe("version string parsers", () => {
  ...
  it("should parse age version from real output", () => {
    expect(parseAgeVersion("v1.1.1")).toBe("1.1.1");
  });

  it("should parse age version with v prefix", () => {
    expect(parseAgeVersion("v1.3.1")).toBe("1.3.1");
  });

  it("should return null for unparseable age output", () => {
    expect(parseAgeVersion("unknown")).toBeNull();
  });
...
describe("checkDependency", () => {
  ...
  it("should check age version correctly", async () => {
    const runner = makeRunner({
      age: { stdout: "v1.3.1", stderr: "", exitCode: 0 },
    });

    const result = await checkDependency("age", runner);

    expect(result).not.toBeNull();
    expect(result!.installed).toBe("1.3.1");
    expect(result!.satisfied).toBe(true);
  });
...
describe("checkAll", () => {
  it("should run all three checks in parallel", async () => {
    const runner: SubprocessRunner = {
    ...
    expect(status.age).not.toBeNull();
    expect(status.git).not.toBeNull();
    expect(status.sops!.satisfied).toBe(true);
    expect(status.age!.satisfied).toBe(true);
    expect(status.git!.satisfied).toBe(true);
    // Verify all three were called (parallel execution via Promise.all)
...
describe("assertAge", () => {
  it("should resolve when age is installed and version is satisfied", async () => {
    const runner = makeRunner({
      age: { stdout: "v1.1.1", stderr: "", exitCode: 0 },
    });

    await expect(assertAge(runner)).resolves.toBeUndefined();
  });

  it("should throw AgeMissingError when age is not found", async () => {
    const runner = makeRunner({});

    await expect(assertAge(runner)).rejects.toThrow(AgeMissingError);
  });

  it("should throw AgeVersionError when age is below minimum", async () => {
    const runner = makeRunner({
      age: { stdout: "v1.0.0", stderr: "", exitCode: 0 },
    });

    const err = await assertAge(runner).catch((e) => e);
    expect(err).toBeInstanceOf(AgeVersionError);
    expect(err.installed).toBe("1.0.0");
    expect(err.required).toBe(REQUIREMENTS.age);
  });
});
...
describe("getInstallHint (platform branches)", () => {
  ...
  it("should return linux-specific hint for age on linux", async () => {
    Object.defineProperty(process, "platform", { value: "linux" });
    const runner = makeRunner({
      age: { stdout: "v1.3.1", stderr: "", exitCode: 0 },
    });
    ...
    const result = await checkDependency("age", runner);
    expect(result).not.toBeNull();
    expect(result!.installHint).toContain("https://github.com/FiloSottile/age/releases");
  });

  it("should return fallback hint for age on win32", async () => {
...
describe("error classes", () => {
  ...
  it("AgeMissingError has installHint", () => {
    const err = new AgeMissingError("brew install age");
    expect(err.installHint).toBe("brew install age");
    expect(err.name).toBe("AgeMissingError");
  });

  it("AgeVersionError has installed and required", () => {
    const err = new AgeVersionError("1.0.0", "1.1.0", "brew upgrade age");
    expect(err.installed).toBe("1.0.0");
    expect(err.required).toBe("1.1.0");
    expect(err.name).toBe("AgeVersionError");
  });
});
