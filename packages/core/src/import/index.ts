...
export class ImportRunner {
  ...
  async import(
    ...
    const ref: CellRef = { namespace: ns, environment: env };
    ...
    if (options.dryRun) {
      // Dry run: check existing keys but never call write
      let existingKeys: Set<string>;
      try {
        const decrypted = await this.source.readCell(ref);
        existingKeys = new Set(Object.keys(decrypted.values));
      } catch {
        // Cell may not exist or be inaccessible — treat as empty
    ...
    // the cell once inside a transaction.
    const decrypted = await this.source.readCell(ref);
