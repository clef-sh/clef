...
Integration tests live in the `integration/` directory at the repo root and are excluded from the default `npm test` run. They require actual `sops` and `age` binaries installed on your machine.
...
If `sops` or `age` are not found in PATH, the tests will fail with a clear message explaining what to install.
...
correctness with real SOPS and age binaries. Coverage
