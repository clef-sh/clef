...
- **`packages/cli`** — Commander.js CLI wrapping core. Commands: init, get, set, compare, delete, diff, lint, rotate, hooks, exec, export, import, doctor, update, scan, recipients, ui, merge-driver, service, pack, drift, agent. Has `optionalDependencies` on `@clef-sh/sops-{platform}-{arch}` packages for bundled sops binary.
