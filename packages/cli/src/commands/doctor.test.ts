...
describe("clef doctor", () => {
  ...
  it("should output valid JSON with --json flag and count age recipients", async () => {
    ...
    expect(parsed.age.version).toBe("1.1.1");
    expect(parsed.age.ok).toBe(true);
  ...
  it("should exit 1 when age is missing", async () => {
    const runner: SubprocessRunner = {
      run: jest.fn().mockImplementation(async (cmd: string) => {
    ...
    const printCalls = mockFormatter.print.mock.calls.map((c) => String(c[0]));
    const ageLine = printCalls.find((l) => l.includes("age") && l.includes("not installed"));
  ...
  it("should detect age key from manifest's age_key_file", async () => {
    const origKey = process.env.SOPS_AGE_KEY;
    const origKeyFile = process.env.SOPS_AGE_KEY_FILE;
    delete process.env.SOPS_AGE_KEY;
    delete process.env.SOPS_AGE_KEY_FILE;
    ...
    mockFs.existsSync.mockImplementation((p: fs.PathLike) => {
      ...
      // Manifest's age_key_file exists
      if (s.includes(".sops/keys.txt")) return true;
      return true;
    });
  ...
  it("should return platform-specific install hints", async () => {
    // This test covers getSopsInstallHint and getAgeInstallHint
    // On darwin, they return brew hints. On other platforms, URL hints.
    // We test by having sops and age missing, which triggers the hint display.
    const runner: SubprocessRunner = {
      run: jest.fn().mockImplementation(async (cmd: string) => {
        if (cmd === "git") return { stdout: "git version 2.43.0", stderr: "", exitCode: 0 };
        return { stdout: "", stderr: "not found", exitCode: 127 };
    ...
    // Should have hint lines for both sops and age
  ...
  it("should show URL hints on non-darwin platforms when sops/age missing", async () => {
    ...
    const urlHints = hintCalls.filter(
      (l) =>
        l.includes("https://github.com/getsops/sops") ||
        l.includes("https://github.com/FiloSottile/age"),
    );
    expect(urlHints.length).toBeGreaterThanOrEqual(2);
