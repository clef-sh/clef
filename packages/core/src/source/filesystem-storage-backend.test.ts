...
function makeTempRoot(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "clef-fs-storage-backend-"));
}
...
describe("FilesystemStorageBackend", () => {
  ...
  describe("cellPath", () => {
    it("substitutes {namespace} and {environment} in file_pattern", () => {
      const store = new FilesystemStorageBackend(manifest(), root);
    ...
    it("honors a JSON file_pattern", () => {
      ...
      const store = new FilesystemStorageBackend(m, root);
      const p = store.cellPath({ namespace: "api", environment: "dev" });
  ...
  describe("identifiers", () => {
    it("id is 'filesystem'", () => {
      expect(new FilesystemStorageBackend(manifest(), root).id).toBe("filesystem");
    });
  });
});
