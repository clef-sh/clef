...
describe("ServiceIdentityManager", () => {
  ...
  describe("deleteKeys", () => {
    ...
    it("should remove the identity's key from keystore files", async () => {
      const manifest = manifestWithKeystore();
      mockFs.existsSync.mockReturnValue(true);
      encryption.decrypt.mockResolvedValue({
        values: { "my-svc": "AGE-SECRET-KEY-1", "other-svc": "AGE-SECRET-KEY-2" },
        metadata: { backend: "age", recipients: [], lastModified: new Date() },
      });

      await manager.deleteKeys("my-svc", manifest, "/repo");

      // Should re-encrypt without the deleted key
      expect(encryption.encrypt).toHaveBeenCalledWith(
        expect.stringContaining("_keystore/"),
        { "other-svc": "AGE-SECRET-KEY-2" },
        manifest,
        expect.any(String),
      );
    });

    it("should no-op when keystore namespace does not exist", async () => {
      const manifest = baseManifest(); // no _keystore namespace
      await manager.deleteKeys("my-svc", manifest, "/repo");
      expect(encryption.decrypt).not.toHaveBeenCalled();
    });

    it("should no-op when identity not in keystore file", async () => {
      const manifest = manifestWithKeystore();
      mockFs.existsSync.mockReturnValue(true);
      encryption.decrypt.mockResolvedValue({
        values: { "other-svc": "AGE-SECRET-KEY-2" },
        metadata: { backend: "age", recipients: [], lastModified: new Date() },
      });

      await manager.deleteKeys("missing-svc", manifest, "/repo");

      // Should not re-encrypt since key wasn't found
      expect(encryption.encrypt).not.toHaveBeenCalled();
    });

    it("should skip environments where keystore file does not exist", async () => {
      const manifest = manifestWithKeystore();
      mockFs.existsSync.mockReturnValue(false);
  ...
  describe("hasStoredKeys", () => {
    ...
    it("should return true for environments with stored keys", async () => {
      const manifest = manifestWithKeystore();
      mockFs.existsSync.mockReturnValue(true);
      encryption.decrypt.mockResolvedValue({
        values: { "my-svc": "AGE-SECRET-KEY-1" },
        metadata: { backend: "age", recipients: [], lastModified: new Date() },
      });

      const result = await manager.hasStoredKeys("my-svc", manifest, "/repo");
      expect(result.dev).toBe(true);
      expect(result.staging).toBe(true);
      expect(result.production).toBe(true);
    });

    it("should return false for environments without stored keys", async () => {
      const manifest = manifestWithKeystore();
      mockFs.existsSync.mockReturnValue(true);
      encryption.decrypt.mockResolvedValue({
        values: { "other-svc": "AGE-SECRET-KEY-2" },
    ...
    it("should return all false when keystore namespace does not exist", async () => {
      const manifest = baseManifest(); // no _keystore namespace
    ...
    it("should return false when keystore file does not exist", async () => {
      const manifest = manifestWithKeystore();
      mockFs.existsSync.mockReturnValue(false);
