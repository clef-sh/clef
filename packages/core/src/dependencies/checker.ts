...
/**
 * Extract version from age output.
 * Format: "v1.1.1" (just the version with v prefix)
 */
function parseAgeVersion(stdout: string): string | null {
 ...
 */
function getInstallHint(name: "sops" | "age" | "git"): string {
  ...
  switch (name) {
    ...
    case "age":
      if (platform === "darwin") return "brew install age";
      if (platform === "linux") return "see https://github.com/FiloSottile/age/releases";
  ...
  try {
    ...
    switch (name) {
      case "sops":
        ...
        break;
      case "age":
        installed = parseAgeVersion(output);
        break;
      case "git":
...
/**
 * Assert that age is installed and meets the minimum version.
 * Throws AgeMissingError or AgeVersionError.
 */
export async function assertAge(runner: SubprocessRunner): Promise<void> {
  const dep = await checkDependency("age", runner);

  if (!dep) {
    throw new AgeMissingError(getInstallHint("age"));
  }

  if (!dep.satisfied) {
    throw new AgeVersionError(dep.installed, dep.required, getInstallHint("age"));
  }
}

// Exported for testing
export { parseSopsVersion, parseAgeVersion, parseGitVersion, semverSatisfied };
