...
describe("formatter", () => {
  ...
  describe("formatDependencyError", () => {
    ...
    it("should format AgeMissingError correctly", () => {
      const err = new AgeMissingError("brew install age");
      formatter.formatDependencyError(err);

      const output = stderrWrite.mock.calls.map((c) => String(c[0])).join("");
      expect(output).toContain("age is not installed");
      expect(output).toContain("brew install age");
    });
    ...
    it("should format AgeVersionError correctly", () => {
      ...
      expect(output).toContain("brew upgrade age");
    });
