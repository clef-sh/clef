...
The default pattern generated by `clef init` is `secrets/{namespace}/{environment}.enc.yaml`.

### Service identities

Service identities allow machine workloads (Lambda, Cloud Run, containers) to consume secrets at runtime without git or the `sops` binary. Each identity declares namespace scope and per-environment age public keys.
